...
test('paid sale correction preserves payment and updates the existing sale ledger in one transaction',async t=>{
 ...
 const sale={_id:id,tenantId:id,musteriId:cid,depoId:id,genelToplam:120,odenenTutar:100,kalanTutar:20,belgeNo:'TEST',kalemler:[{urunId:pid,miktar:1}],toObject(){return {genelToplam:this.genelToplam,odenenTutar:this.odenenTutar};},save:async opts=>assert.equal(opts.session,session)};
 ...
 t.mock.method(require('../src/services/satisCariEsitlemeServisi'),'esitle',async opts=>{assert.ok(inTransaction);assert.equal(opts.session,session);assert.equal(opts.satis.genelToplam,240);customer.bakiye=150;return {musteriBakiye:150};});
 ...
 assert.ifError(error);assert.equal(response.satis.genelToplam,240);assert.equal(sale.odenenTutar,100);assert.equal(sale.kalanTutar,140);assert.equal(customer.bakiye,150);assert.equal(stock.miktar,8);assert.equal(ledger.length,0);assert.equal(audits.length,1);assert.ok(ended);
 // A total below the paid amount must fail before any stock/ledger write.
 response=null;error=null;await require('../src/controllers/satisController').guncelle({tenantId:id,params:{id},currentUser:{rol:'ADMIN'},body:{kalemler:[{urunId:pid,miktar:1,birimFiyat:10,kdv:20,iskonto:0}]}},res,e=>{error=e;});assert.equal(error.status,409);assert.equal(stock.miktar,8);assert.equal(ledger.length,0);
});
