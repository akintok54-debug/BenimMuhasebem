...
const Portfoy=require('../src/models/CekSenetPortfoy');
...
function fixture(t,extra=[]){
 ...
 t.mock.method(Portfoy,'find',f=>{assert.equal(f.tenantId,'tenant');return query(portfolio.filter(r=>matches(r,f)));});
 ...
 return {rows,customers,portfolio,session,satis:{_id:'sale',tenantId:'tenant',musteriId:'old',genelToplam:8829.80,durum:'AKTIF'}};
}
...
test('customer transfer keeps cheque source association and preserves face value and drawer',async t=>{const f=fixture(t);f.portfolio.push({_id:'cheque',tenantId:'tenant',kaynak:'SATIS',kaynakId:'sale',musteriId:'old',tutar:100,kesideci:'Original drawer'});f.satis.musteriId='new';await esitle(f);assert.equal(f.portfolio[0].musteriId,'new');assert.equal(f.portfolio[0].tutar,100);assert.equal(f.portfolio[0].kesideci,'Original drawer');});
test('conflicting cheque customer aborts before ledger change',async t=>{const f=fixture(t);f.portfolio.push({_id:'cheque',tenantId:'tenant',kaynak:'SATIS',kaynakId:'sale',musteriId:'unrelated'});f.satis.musteriId='new';await assert.rejects(esitle(f),{status:409});assert.equal(f.rows[0].tutar,20312.28);});
