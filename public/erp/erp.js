(() => {
    ...
    async function hizliSatisUrunuAc(secenekler = {}) {
        const depolar = secenekler.depolar || (await api("/api/tenant/stok/depolar")).depolar || [];
    ...
    function alisGenelRaporu(panel, donem = "BU_AY", ozelBaslangic = "", ozelBitis = "") {
        ...
        const rows = alisMerkezi.alislar.filter(x => { const tarih = String(x.tarih || "").slice(0, 10); return (!aralik.baslangic || tarih >= aralik.baslangic) && (!aralik.bitis || tarih <= aralik.bitis); });
    ...
    const RAPOR_KOD_ETIKETLERI = {
        ...
        BORC: "Borç", ALACAK: "Alacak", MANUEL: "Manuel İşlem", PORTFOYDE: "Portföyde",
    ...
    function eticaretBaglantiFormu(mevcut=null) {
        const providers=["TRENDYOL","HEPSIBURADA","N11","AMAZON_TR","CICEKSEPETI","PAZARAMA","EDOCUMENT_CUSTOM"], overlay=document.createElement("div"); let secili=mevcut?.provider||"TRENDYOL", kayitId=mevcut?._id||"";
        overlay.className="erp-modal-overlay"; overlay.id="eticaretModal";
        overlay.innerHTML=`<div class="erp-modal ecommerce-connect-modal"><div class="erp-modal-header"><div><small>GÜVENLİ ENTEGRASYON</small><h2>${mevcut?"Hesap Ayarları":"Hesap Bağla"}</h2><p>Sağlayıcınızı seçin ve bağlantı bilgilerini doğrulayın.</p></div><button class="erp-modal-close" aria-label="Kapat">×</button></div><form><div class="ecommerce-provider-grid">${providers.map(k=>`<button type="button" class="ecommerce-provider-card ${secili===k?"active":""}" data-provider="${k}"><span>${k==="EDOCUMENT_CUSTOM"?"EB":k.slice(0,2)}</span><b>${escapeHtml(eticaretProviderEtiketi[k])}</b></button>`).join("")}</div><input type="hidden" name="provider" value="${secili}"><div class="erp-form-grid ecommerce-credential-fields"><label class="full">Mağaza / Hesap Adı<input name="storeName" required value="${escapeHtml(mevcut?.storeName||"")}" placeholder="Örn. Akın Motor"></label><label data-seller-label>Seller ID<input name="sellerId" value="${escapeHtml(mevcut?.sellerId||mevcut?.merchantId||"")}" autocomplete="off"></label><label>Ortam<select name="environment"><option value="STAGE" ${mevcut?.environment==="STAGE"?"selected":""}>Test</option><option value="PRODUCTION" ${mevcut?.environment!=="STAGE"?"selected":""}>Production</option></select></label><label>API Key / Kullanıcı<input name="apiKey" ${mevcut?"":"required"} autocomplete="off" placeholder="${mevcut?"Değişmeyecekse boş bırakın":"API Key"}"></label><label>API Secret<input name="apiSecret" type="password" ${mevcut?"":"required"} autocomplete="new-password" placeholder="********"></label><label class="full">Token <small>Yalnızca sağlayıcınız istiyorsa</small><input name="token" type="password" autocomplete="new-password" placeholder="********"></label></div><div class="ecommerce-secret-note">API secret ve token değerleri maskeli tutulur; ekranda veya loglarda gösterilmez.</div><div id="eticaretFormMesaj" aria-live="polite"></div><div class="erp-modal-footer"><button type="button" class="erp-small-button" data-kapat>Vazgeç</button><button class="erp-primary-button" data-test-submit>${mevcut?"Kaydet ve Bağlantıyı Test Et":"Bağlantıyı Test Et"}</button></div></form></div>`;
        ...
        const alanlariGuncelle=()=>{overlay.querySelector("[data-seller-label]").childNodes[0].textContent=secili==="EDOCUMENT_CUSTOM"?"Hesap / Mükellef ID":"Seller ID";}; alanlariGuncelle();
        ...
        form.onsubmit=async event=>{event.preventDefault();const submit=form.querySelector("[data-test-submit]");submit.disabled=true;mesaj.innerHTML='<div class="ecommerce-inline-status">Bağlantı güvenli biçimde doğrulanıyor...</div>';const fd=new FormData(form),credentials={};for(const key of ["apiKey","apiSecret","token"])if(fd.get(key))credentials[key]=fd.get(key);const body={type:secili==="EDOCUMENT_CUSTOM"?"EDOCUMENT":"MARKETPLACE",provider:secili,storeName:fd.get("storeName"),sellerId:fd.get("sellerId"),environment:fd.get("environment")};if(Object.keys(credentials).length)body.credentials=credentials;try{const saved=await api(kayitId?`/api/tenant/eticaret/connections/${kayitId}`:"/api/tenant/eticaret/connections",{method:kayitId?"PATCH":"POST",body:JSON.stringify(body)});kayitId=saved.connection?._id||kayitId;await api(`/api/tenant/eticaret/connections/${kayitId}/test`,{method:"POST"});mesaj.innerHTML='<div class="ecommerce-connect-result success"><b>✓ Bağlantı başarılı</b><span>Hesap güvenli biçimde kaydedildi ve doğrulandı.</span></div>';submit.textContent="Bağlantıyı Yeniden Test Et";}catch(error){mesaj.innerHTML=`<div class="ecommerce-connect-result danger"><b>Bağlantı kurulamadı</b><span>${escapeHtml(error.message)}</span></div>`;}finally{submit.disabled=false;}};
    }
    ...
    async function eticaretYukle(aktifSekme="overview", filtre={}) {
        ...
        try {
            ...
            const renderConnections=()=>{panel.innerHTML=`<div class="panel-heading ecommerce-section-heading"><div><h2>Pazaryeri ve E-Belge Hesapları</h2><p>Bağlı hesapları, durumlarını ve son senkronizasyonlarını yönetin.</p></div></div><div class="ecommerce-account-grid">${connections.map((x,i)=>`<article class="ecommerce-account-card"><div class="ecommerce-account-head"><span class="ecommerce-provider-mark">${escapeHtml((eticaretProviderEtiketi[x.provider]||x.provider).slice(0,2))}</span><div><small>${escapeHtml(eticaretProviderEtiketi[x.provider]||x.provider)}</small><h3>${escapeHtml(x.storeName)}</h3></div>${eticaretBadge(x.active?"ACTIVE":"INACTIVE",x.active?"Bağlı":"Devre Dışı")}</div><dl><div><dt>Satıcı / Hesap</dt><dd>${escapeHtml(x.sellerId||x.merchantId||"-")}</dd></div><div><dt>Ortam</dt><dd>${x.environment==="STAGE"?"Test":"Production"}</dd></div><div><dt>Son senkronizasyon</dt><dd>${eticaretTarihSaat(x.lastSuccessfulSyncAt)}</dd></div></dl><div class="ecommerce-card-actions">${oturumYetkisiVar("ecommerce.settings")?`<button class="erp-small-button" data-ec-sync="${i}">Yenile</button><button class="erp-small-button" data-ec-edit="${i}">Ayarlar</button><button class="erp-small-button danger-button-soft" data-ec-disable="${i}" ${x.active?"":"disabled"}>Devre Dışı Bırak</button>`:"<span>Görüntüleme yetkisi</span>"}</div></article>`).join("")||'<div class="dashboard-panel empty-state">Henüz hesap bağlanmadı. “Hesap Bağla” ile başlayabilirsiniz.</div>'}</div>`;};
