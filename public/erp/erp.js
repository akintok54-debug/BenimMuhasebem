(() => {
    ...
    async function musteriBelgeFormu(tur, musteri, mevcut = null, baslangicKalemleri = []) {
        const [urunData, stokData, finansData] = await Promise.all([api("/api/tenant/urunler"), api("/api/tenant/stok/depolar"), tur === "satis" ? (oturumYetkisiVar("cash.read") ? api("/api/tenant/finans/ozet") : api("/api/tenant/saha/panel")) : Promise.resolve({})]);
        ...
        const odemeHtml = tur === "satis" && !mevcut ? `<div class="sales-payment-box"><div><span>Belge Toplamı</span><strong id="salesDocumentTotal">₺0,00</strong></div><label>Ödeme Yöntemi<select name="odemeTipi"><option value="ACIK_HESAP">Açık Hesap</option><option value="NAKIT">Nakit</option><option value="KART">POS / Kredi Kartı</option><option value="BANKA">IBAN / Havale</option><option value="CEK">Çek</option><option value="SENET">Senet</option></select></label><label data-sales-account hidden>Kasa / Banka<select name="hesap"><option value="">Hesap seçin</option>${satisHesaplari.map(x => `<option value="${x.tip}|${x.id}" data-hesap-tipi="${x.tip}">${escapeHtml(x.tip)} · ${escapeHtml(x.ad)}</option>`).join("")}</select></label></div>` : "";
    ...
    async function satisPaneliYukle() {
        ...
        try {
            ...
            <section id="salesPos" class="sales-pos"><div class="sales-catalog"><div class="panel-heading"><div><h2>Ürün Seçimi</h2><p>Kod, barkod veya ürün adıyla arayın; miktarı yazıp sepete ekleyin.</p></div><div class="sales-product-search-wrap"><input id="salesProductSearch" class="erp-input" placeholder="Ürün adı, kod veya barkod ara..." autocomplete="off"><small id="salesProductResultCount">${katalog.length} ürün gösteriliyor</small></div></div><div class="sales-product-grid">${katalog.map(u => { const stok = stokHaritasi.get(String(u._id)) || 0, arama = [u.kod,u.barkod,u.ad,u.marka,u.model,u.kategori].filter(Boolean).join(" "); return `<article class="sales-product-card" data-sales-product="${u._id}" data-sales-search="${escapeHtml(arama)}"><span>${escapeHtml(u.kod || "ÜRÜN")}${u.barkod ? ` · ${escapeHtml(u.barkod)}` : ""}</span><b>${escapeHtml(u.ad)}</b><small>${escapeHtml([u.marka,u.model].filter(Boolean).join(" · ") || u.birim || "ADET")} · Stok <strong>${stok}</strong> ${escapeHtml(u.birim || "ADET")}</small><strong class="sales-product-price">${para(u.satisFiyati)}</strong><div class="sales-product-add"><label>Miktar<input type="number" min="1" max="${Math.max(1,stok)}" step="1" value="1" inputmode="numeric" data-sales-quantity="${u._id}" ${stok <= 0 ? "disabled" : ""}></label><button type="button" data-sales-add="${u._id}" ${stok <= 0 ? "disabled" : ""}>${stok > 0 ? "Sepete Ekle" : "Stok Yok"}</button></div></article>`; }).join("") || '<div class="empty-state">Satışa uygun aktif ürün bulunamadı.</div>'}<div id="salesProductEmpty" class="empty-state" hidden>Aramanızla eşleşen ürün bulunamadı.</div></div></div><aside class="sales-cart"><div><span>HIZLI SATIŞ</span><h2>Satış Sepeti</h2></div><div id="salesCartItems" class="sales-cart-items"><div class="empty-state">Henüz ürün eklenmedi.</div></div><div class="sales-cart-total"><span>Sepet Toplamı</span><strong id="salesCartTotal">₺0,00</strong></div><button id="salesChooseCustomer" class="erp-primary-button" disabled>Müşteri Seç ve Satışa Başla</button><small>Müşteri seçildikten sonra depo ve ödeme bilgilerini tamamlayabilirsiniz.</small></aside></section>
            ...
            const sepet = new Map();
            const sepetCiz = () => { const satirlar = [...sepet.values()]; content.querySelector("#salesCartItems").innerHTML = satirlar.map(x => `<div class="sales-cart-row"><div><b>${escapeHtml(x.ad)}</b><small>${escapeHtml(x.kod)} · ${para(x.satisFiyati)}</small></div><div class="sales-cart-quantity"><button type="button" data-cart-minus="${x._id}">−</button><input type="number" min="1" max="${stokHaritasi.get(String(x._id)) || 1}" step="1" value="${x.miktar}" data-cart-quantity="${x._id}" aria-label="${escapeHtml(x.ad)} miktarı"><button type="button" data-cart-plus="${x._id}">+</button></div><button type="button" class="sales-cart-remove" data-cart-remove="${x._id}">×</button></div>`).join("") || '<div class="empty-state">Henüz ürün eklenmedi.</div>'; content.querySelector("#salesCartTotal").textContent = para(satirlar.reduce((n, x) => n + x.miktar * Number(x.satisFiyati || 0) * (1 + Number(x.kdv ?? 20) / 100), 0)); content.querySelector("#salesChooseCustomer").disabled = !satirlar.length; content.querySelectorAll("[data-cart-minus]").forEach(b => b.onclick = () => { const x = sepet.get(b.dataset.cartMinus); if (x.miktar <= 1) sepet.delete(b.dataset.cartMinus); else x.miktar--; sepetCiz(); }); content.querySelectorAll("[data-cart-plus]").forEach(b => b.onclick = () => { const x = sepet.get(b.dataset.cartPlus), stok = stokHaritasi.get(String(x._id)) || 0; if (x.miktar < stok) x.miktar++; sepetCiz(); }); content.querySelectorAll("[data-cart-quantity]").forEach(input => input.onchange = () => { const x = sepet.get(input.dataset.cartQuantity), stok = stokHaritasi.get(String(x._id)) || 0; x.miktar = Math.max(1, Math.min(stok, Math.floor(Number(input.value || 1)))); sepetCiz(); }); content.querySelectorAll("[data-cart-remove]").forEach(b => b.onclick = () => { sepet.delete(b.dataset.cartRemove); sepetCiz(); }); };
            const sepeteEkle = id => { const u = katalog.find(x => String(x._id) === String(id)), stok = stokHaritasi.get(String(id)) || 0, input = content.querySelector(`[data-sales-quantity="${CSS.escape(String(id))}"]`); if (!u || stok <= 0) return; const miktar = Math.max(1, Math.min(stok, Math.floor(Number(input?.value || 1)))), mevcut = sepet.get(String(id)); if (mevcut) mevcut.miktar = Math.min(stok, mevcut.miktar + miktar); else sepet.set(String(id), { ...u, miktar }); if (input) input.value = "1"; sepetCiz(); };
    ...
    function sahaIcerikCiz(d) {
        ...
        const secimler = d.yonetici ? `<select id="sahaTemsilci"><option value="">Kendi hesabım</option>${(d.temsilciler || []).map(x => `<option value="${x._id}" ${String(x._id) === String(d.seciliKullaniciId) ? "selected" : ""}>${escapeHtml(x.adSoyad || x.email)}</option>`).join("")}</select>` : "";
        ...
        <section class="dashboard-panel saha-customers"><div class="panel-heading"><div><h2>Müşterilerim</h2><p>GPS kayıtlı müşteri kartı, ziyaret, satış ve ekstre paylaşımı.</p></div><input id="sahaMusteriAra" placeholder="Müşteri ara..."></div><div class="saha-customer-grid">${(d.musteriler || []).map(m => `<article data-saha-musteri-kart><div><strong>${escapeHtml(sahaMusteriAdi(m))}</strong><small>${escapeHtml(m.kod)} · ${escapeHtml(m.il || m.ilce || "Konum bilgisi yok")}</small></div><div class="saha-customer-actions"><button data-sm="ziyaret" data-id="${m._id}">Ziyaret</button><button data-sm="satis" data-id="${m._id}">Satış</button><button data-sm="ekstre" data-id="${m._id}">PDF Ekstre</button><button data-sm="wa" data-id="${m._id}">WhatsApp</button><button data-sm="duzenle" data-id="${m._id}">Düzenle</button></div></article>`).join("") || '<div class="empty-state">Henüz size atanmış müşteri bulunmuyor.</div>'}</div></section>`;
        ...
        document.querySelectorAll("[data-sm]").forEach(x => x.onclick = async () => { const m=d.musteriler.find(y=>String(y._id)===x.dataset.id); if(x.dataset.sm==="duzenle")return sahaMusteriFormu(m); if(x.dataset.sm==="satis")return musteriBelgeFormu("satis",m).catch(e=>alert(e.message)); if(x.dataset.sm==="ekstre")return cariEkstreAc("musteri",m._id); if(x.dataset.sm==="wa")return sahaEkstreWhatsapp(m).catch(e=>alert(e.message)); if(x.dataset.sm==="ziyaret"){ try{await api("/api/tenant/saha/ziyaret/baslat",{method:"POST",body:JSON.stringify({gun:d.gun,musteriId:m._id,konum:await sahaGpsAl()})});await sahaYukle(d.gun,d.seciliKullaniciId)}catch(e){alert(e.message)} } });
        document.querySelectorAll("[data-saha]").forEach(x => x.onclick = () => sahaAksiyon(x.dataset.saha,d));
    }
