(() => {
    ...
    async function tedarikciBelgeFormu(tur, tedarikci, donus = "tedarikci") {
        ...
        overlay.querySelector("#tedKalemEkle").onclick = event => {
            ...
            const yeniSatir = tbody.lastElementChild; bagla(yeniSatir);
    ...
    async function satisPaneliYukle() {
        ...
        try {
            ...
            const sepet = new Map(); let satisModu = "NORMAL";
            ...
            content.querySelector("#salesChooseCustomer").onclick = () => { const kalemler = [...sepet.values()].map(x => ({ urunId: x._id, miktar: x.miktar, birimFiyat: Number(x.birimFiyat || 0), kdv: Number(x.kdv ?? 20), iskonto: satisModu === "PERAKENDE" ? 0 : Number(x.iskonto || 0) })); if (satisModu === "PERAKENDE") musteriBelgeFormu("satis", { kod: "PERAKENDE", unvan: "Perakende Müşteri", bakiye: 0 }, null, kalemler, { perakende: true }).catch(error => alert(error.message)); else satisMusteriSec("satis", kalemler); };
    ...
    async function finansHareketGecmisiRender(panel, data, hesapMap) {
        ...
        panel.querySelector("#finansGecmisGetir").onclick = getir;
    ...
    async function masraflarYukle() {
        setTitle("Masraflar"); loading("Masraf kayıtları hazırlanıyor...");
        try {
            const [liste, ozet, finans] = await Promise.all([api("/api/tenant/masraflar?limit=500"), api("/api/tenant/masraflar/ozet"), api("/api/tenant/finans/ozet")]);
    ...
    function sahaIcerikCiz(d) {
        ...
        <div class="dashboard-grid saha-kpis">${card("Ziyaret", `${tamam}/${Number(g?.gunlukZiyaretHedefi || 0)}`, aktifZ ? "Ziyaret sürüyor" : "Günlük hedef")}${card("Toplam KM", Number(g?.toplamKm || 0), g ? `${Number(g.cikisKm || 0)} → ${g.donusKm ?? "-"}` : "Araç çıkışı bekliyor")}${card("Net Ciro", para(t.netCiro), `${Number(t.satisAdedi || 0)} satış`)}${card("Ciro Primi", para(g?.hesaplananPrim || 0), g?.ciroHedefi ? `Hedef ${para(g.ciroHedefi)}` : "Gün sonunda hesaplanır")}${card("Teslim Kasa", para(t.teslimEdilmesiGereken), k.durum || "BEKLİYOR")}</div>
        ...
        <aside class="dashboard-panel"><h2>Tesellüm Özeti</h2><div class="saha-tesellum">${[["Ciro",t.ciro],["Nakit",t.nakit],["POS / Kart",t.posKrediKarti],["IBAN",t.iban],["Çek",t.cek],["Senet",t.senet],["Açık hesap",t.acikHesap],["İade",t.iadeler],["Mazot",t.masraflar?.mazot],["Yemek",t.masraflar?.yemek],["Diğer",t.masraflar?.diger]].map(x=>`<div><span>${x[0]}</span><b>${para(x[1])}</b></div>`).join("")}<div class="total"><span>Teslim gereken</span><b>${para(t.teslimEdilmesiGereken)}</b></div>${k.teslimTarihi ? `<div class="total ${k.durum === "TAM" ? "ok" : "warn"}"><span>${escapeHtml(k.durum)}</span><b>${para(k.fark)}</b></div>` : ""}</div></aside></div>
    ...
    function raporHucre(value, key = "") {
        ...
        if (typeof value === "number") return /miktar|sayisi|adet/i.test(key) ? value.toLocaleString("tr-TR") : para(value);
        if (typeof value === "object") return escapeHtml(value.adSoyad || value.unvan || value.ad || value.kod || JSON.stringify(value));
    ...
    function raporSatirlariniHazirla(rapor) {
        if (rapor.kod === "donemRaporu") return (profesyonelRaporDurumu?.donemRaporu || []).map(x => ({ İşaret: x.isaret, Kalem: x.ad, Tutar: Number(x.tutar || 0) }));
        return (rapor.satirlar || []).map(x => { const sonuc = {}; for (const [key, value] of Object.entries(x)) { if (key.startsWith("_") || key === "__v") continue; const baslik = key.replace(/([A-Z])/g, " $1").replace(/^./, c => c.toLocaleUpperCase("tr-TR")); sonuc[baslik] = value; } return sonuc; });
    }
    ...
    function raporSeciliCiz() {
        ...
        if (!d) return; const rapor = kod === "donemRaporu" ? { kod, ad: "Dönem Kâr ve Stok Raporu", toplam: d.degerler?.netKarZarar || 0, satirlar: d.donemRaporu } : d.raporlar[kod];
        const alan = document.getElementById("raporDetayAlan"); if (alan) alan.innerHTML = `<div class="panel-heading"><div><h2>${escapeHtml(rapor.ad)}</h2><p>${escapeHtml(d.meta.donem.baslangicYazi)} – ${escapeHtml(d.meta.donem.bitisYazi)} · Gerçek kayıtlardan hesaplandı</p></div><b>${para(rapor.toplam || 0)}</b></div>${raporTablosu(rapor)}`;
    }
    function raporExcelIndir() {
        ...
        const bilgi = [[d.meta.firmaAdi], [rapor.ad], [`Tarih aralığı: ${d.meta.donem.baslangicYazi} - ${d.meta.donem.bitisYazi}`], [`Filtreler: ${filtre}`], [`Oluşturulma: ${new Date(d.meta.olusturulmaTarihi).toLocaleString("tr-TR")}`], [], ...XLSX.utils.sheet_to_json(XLSX.utils.json_to_sheet(satirlar), { header: 1 }), [], ["TOPLAM", Number(rapor.toplam || 0)], ["Sayfa", "1 / 1"]];
    ...
    function raporYazdir() {
        ...
        pencere.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${escapeHtml(rapor.ad)}</title><style>@page{size:A4 landscape;margin:15mm}@media print{footer{position:fixed;bottom:0}.no-print{display:none}}body{font:12px Arial;color:#172033}h1{margin-bottom:4px}.meta{color:#596579;margin-bottom:18px}table{width:100%;border-collapse:collapse}th,td{padding:7px;border:1px solid #ccd3df;text-align:left}th{background:#edf2f7}tfoot{font-weight:bold}footer{margin-top:20px;width:100%;display:flex;justify-content:space-between;color:#667085}.page:after{content:"Sayfa " counter(page) " / " counter(pages)}</style></head><body><h1>${escapeHtml(d.meta.firmaAdi)}</h1><h2>${escapeHtml(rapor.ad)}</h2><div class="meta">Tarih: ${escapeHtml(d.meta.donem.baslangicYazi)} – ${escapeHtml(d.meta.donem.bitisYazi)}<br>Filtreler: ${escapeHtml(filtre)}<br>Oluşturulma: ${new Date(d.meta.olusturulmaTarihi).toLocaleString("tr-TR")}</div>${raporTablosu(rapor)}<footer><span>Toplam: ${para(rapor.toplam || 0)}</span><span class="page"></span></footer><script>window.onload=()=>window.print()<\/script></body></html>`); pencere.document.close();
    }
    async function raporMerkeziYukle(yuklemeNo = sayfaYuklemeNo) {
        setTitle("Profesyonel ERP Raporları"); loading("Gerçek kayıtlar ve rapor filtreleri hazırlanıyor...");
        try {
            ...
            content.innerHTML = `<div class="welcome-banner"><div><div class="eyebrow">PROFESYONEL ERP RAPOR MERKEZİ</div><h2>Finansal sonuç ve operasyon analizi</h2><p>Satış, alış, stok, cari, finans, çek-senet ve gider kayıtları tek tenant güvenli raporda.</p></div></div><section class="dashboard-panel"><form id="profesyonelRaporForm" class="erp-form-grid report-filter-grid"><label>Dönem<select name="donem"><option value="BUGUN">Bugün</option><option value="DUN">Dün</option><option value="BU_HAFTA">Bu hafta</option><option value="BU_AY" selected>Bu ay</option><option value="GECEN_AY">Geçen ay</option><option value="BU_YIL">Bu yıl</option><option value="GECEN_YIL">Geçen yıl</option><option value="OZEL">Özel tarih aralığı</option></select></label><label>Başlangıç<input name="baslangic" type="date"></label><label>Bitiş<input name="bitis" type="date"></label><label>Şube<select name="sube"><option value="">Tüm şubeler</option>${(s.subeler || []).map(x => `<option>${escapeHtml(x)}</option>`).join("")}</select></label><label>Depo<select name="depoId"><option value="">Tüm depolar</option>${opts(s.depolar,"ad")}</select></label><label>Müşteri<select name="musteriId"><option value="">Tüm müşteriler</option>${opts(s.musteriler,"unvan")}</select></label><label>Tedarikçi<select name="tedarikciId"><option value="">Tüm tedarikçiler</option>${opts(s.tedarikciler,"unvan")}</select></label><label>Ürün<select name="urunId"><option value="">Tüm ürünler</option>${opts(s.urunler,"ad")}</select></label><label>Marka<select name="marka"><option value="">Tüm markalar</option>${(s.markalar || []).map(x => `<option>${escapeHtml(x)}</option>`).join("")}</select></label><label>Kategori<select name="kategori"><option value="">Tüm kategoriler</option>${(s.kategoriler || []).map(x => `<option>${escapeHtml(x)}</option>`).join("")}</select></label><label>Satış Temsilcisi<select name="temsilciId"><option value="">Tüm temsilciler</option>${opts(s.temsilciler,"adSoyad")}</select></label><label>Ödeme Tipi<select name="odemeTipi"><option value="">Tüm ödeme tipleri</option>${(s.odemeTipleri || []).map(x => `<option>${escapeHtml(x)}</option>`).join("")}</select></label><div class="full report-filter-actions"><button class="erp-primary-button">Raporu Oluştur</button><button type="button" id="raporTemizle" class="erp-small-button">Filtreleri Temizle</button></div></form></section><div id="raporSonuc"><div class="dashboard-loading">Rapor hesaplanıyor...</div></div>`;
            const form = content.querySelector("#profesyonelRaporForm"), sonuc = content.querySelector("#raporSonuc");
            const getir = async () => { const fd = new FormData(form), q = new URLSearchParams(); for (const [k, v] of fd) if (v) q.set(k, v); if (fd.get("donem") === "OZEL" && (!fd.get("baslangic") || !fd.get("bitis"))) return alert("Özel dönem için başlangıç ve bitiş tarihini seçin."); const etiketler = [...form.querySelectorAll("select")].filter(x => x.value).map(x => `${x.closest("label").childNodes[0].textContent.trim()}: ${x.selectedOptions[0].textContent.trim()}`); form.dataset.filtreOzeti = etiketler.join(" · ") || "Tüm kayıtlar"; sonuc.innerHTML='<div class="dashboard-loading">Gerçek hareketlerden rapor hesaplanıyor...</div>'; try { const d = await api(`/api/tenant/raporlar/profesyonel?${q}`); if (yuklemeNo !== sayfaYuklemeNo) return; profesyonelRaporDurumu = d; const o=d.ozet||{}, cmp=d.karsilastirma||{}, kart=(ad,value,key)=>`<article class="summary-card"><span>${escapeHtml(ad)}</span><strong>${para(value||0)}</strong><small class="${Number(cmp[key]?.fark||0)>=0?"sales-clear":"sales-debt"}">${cmp[key]?.yuzde===null?"Önceki dönem veri yok":`${Number(cmp[key]?.fark||0)>=0?"▲":"▼"} %${Math.abs(Number(cmp[key]?.yuzde||0)).toLocaleString("tr-TR")}`}</small></article>`; sonuc.innerHTML=`${d.meta.subeNotu?`<div class="error">${escapeHtml(d.meta.subeNotu)}</div>`:""}<div class="dashboard-grid report-summary-grid">${[["Toplam Satış",o.toplamSatis,"toplamSatis"],["Net Satış",o.netSatis,"netSatis"],["Toplam Alış",o.toplamAlis,"toplamAlis"],["Tahsilat",o.tahsilat,"tahsilat"],["Ödeme",o.odeme,"odeme"],["Gider",o.gider,"gider"],["Brüt Kâr",o.brutKar,"brutKar"],["Net Kâr / Zarar",o.netKarZarar,"netKarZarar"],["Müşteri Alacağı",o.musteriAlacagi,"musteriAlacagi"],["Tedarikçi Borcu",o.tedarikciBorcu,"tedarikciBorcu"],["Kasa",o.kasa,"kasa"],["Banka",o.banka,"banka"],["Stok Değeri",o.stokDegeri,"stokDegeri"]].map(x=>kart(...x)).join("")}</div>${raporGrafikleri(d)}<section class="dashboard-panel"><div class="report-toolbar"><label>Rapor<select id="raporTuru"><option value="donemRaporu">Dönem Kâr ve Stok Raporu</option>${(f.raporlar||[]).map(x=>`<option value="${x.kod}">${escapeHtml(x.ad)}</option>`).join("")}</select></label><button id="raporExcel" class="erp-small-button">Excel İndir</button><button id="raporPdf" class="erp-small-button">PDF İndir</button><button id="raporYazdir" class="erp-small-button">Yazdır</button></div><div id="raporDetayAlan"></div></section>`; sonuc.querySelector("#raporTuru").onchange=raporSeciliCiz; sonuc.querySelector("#raporExcel").onclick=raporExcelIndir; sonuc.querySelector("#raporPdf").onclick=raporYazdir; sonuc.querySelector("#raporYazdir").onclick=raporYazdir; raporSeciliCiz(); } catch(error) { sonuc.innerHTML=`<div class="error">${escapeHtml(error.message)}</div>`; } };
            form.onsubmit = e => { e.preventDefault(); getir(); }; content.querySelector("#raporTemizle").onclick = () => { form.reset(); getir(); }; await getir();
        } catch (error) { errorBox(error); }
    }
