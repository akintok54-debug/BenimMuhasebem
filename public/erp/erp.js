(() => {
    ...
    async function personelFinansDetayAc(personelId) {
        ...
        try {
            ...
            if (!hesaplar.length) { const finans = await api(`/api/tenant/finans/ozet?_=${Date.now()}`); hesaplar = [...(finans.kasalar || []).map(x => ({ ...x, tip: "KASA", adGoster: x.ad })), ...(finans.bankalar || []).map(x => ({ ...x, tip: "BANKA", adGoster: x.bankaAdi }))].filter(x => x.aktif !== false); }
