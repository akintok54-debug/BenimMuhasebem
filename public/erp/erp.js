(() => {
    ...
    function sahaIcerikCiz(d) {
        ...
        document.querySelectorAll("[data-sm]").forEach(x => x.onclick = async () => { const m=d.musteriler.find(y=>String(y._id)===x.dataset.id); if(x.dataset.sm==="detay")return sahaMusteriDetayAc(m,d); if(x.dataset.sm==="tahsilat")return sahaTahsilatFormu(m,d); if(x.dataset.sm==="siparis")return musteriBelgeFormu("siparis",m).catch(e=>alert(e.message)); if(x.dataset.sm==="duzenle")return sahaMusteriFormu(m); if(x.dataset.sm==="satis")return musteriBelgeFormu("satis",m).catch(e=>alert(e.message)); if(x.dataset.sm==="ekstre")return cariEkstreAc("musteri",m._id); if(x.dataset.sm==="wa")return sahaEkstreWhatsapp(m).catch(e=>alert(e.message)); if(x.dataset.sm==="ziyaret"){ try{await api("/api/tenant/saha/ziyaret/baslat",{method:"POST",body:JSON.stringify({gun:d.gun,musteriId:m._id,konum:await sahaGpsAl()})});await sahaYukle(d.gun,d.seciliKullaniciId)}catch(e){alert(e.message)} } });
