(() => {
    ...
    async function musteriBelgeMerkeziAc(tur, belge, musteri) {
        ...
        if (["TEKLIF", "SIPARIS"].includes(tur)) {
    ...
    async function finansHesapFormu(tip, mevcut = null) {
        ...
        if (!banka) overlay.querySelector('[name="ad"]').closest("label").insertAdjacentHTML("afterend", `<label>Kasa Türü<select name="kasaTuru"><option value="NAKIT" ${(v.kasaTuru || "NAKIT") === "NAKIT" ? "selected" : ""}>Nakit Kasa</option><option value="DIGER" ${v.kasaTuru === "DIGER" ? "selected" : ""}>Diğer Kasa</option></select></label>`);
        document.body.appendChild(overlay); const kapat = () => overlay.remove(); overlay.querySelectorAll(".erp-modal-close,[data-kapat]").forEach(x => x.onclick = kapat);
    ...
    async function personelFinansDetayAc(personelId) {
        ...
        try {
            const [data, finans] = await Promise.all([api(`/api/tenant/personeller/${personelId}/finans`), api("/api/tenant/finans/ozet")]);
