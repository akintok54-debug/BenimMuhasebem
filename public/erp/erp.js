(() => {
    ...
    async function satisPaneliYukle() {
        ...
        try {
            const [data, urunData, stokData, depoData] = await Promise.all([api("/api/tenant/satis/panel"), api("/api/tenant/urunler"), api("/api/tenant/stok"), api("/api/tenant/stok/depolar")]), p = data.panel || {};
            const son = p.sonSatislar || [], cokSatanlar = p.enCokSatanlar || [], temsilciler = p.temsilciler || [];
            const finansBugun = p.bugun?.finansHareketleri || [], depolar = (depoData.depolar || []).filter(x => x.aktif !== false);
            ...
            <div class="sales-kpis"><article><span>Bugünkü Ciro</span><strong>${para(p.bugun?.ciro)}</strong><small>${Number(p.bugun?.belge || 0)} satış belgesi</small></article><article><span>Bugünkü Tahsilat</span><strong>${para(p.bugun?.tahsilat)}</strong><small>Tüm cari tahsilatlar</small></article><article><span>Bugünkü Ödeme</span><strong>${para(p.bugun?.odeme)}</strong><small>Tedarikçi ödemeleri</small></article><article><span>Aylık Net Ciro</span><strong>${para(p.ay?.netCiro)}</strong><small>${para(p.ay?.iade)} iade düşüldü</small></article><article class="warning"><span>Açık Satış Bakiyesi</span><strong>${para(p.acikBakiye)}</strong><small>Tahsilat bekleyen tutar</small></article><article><span>Satış Hunisi</span><strong>${Number(p.aktifTeklif || 0)} / ${Number(p.acikSiparis || 0)}</strong><small>Aktif teklif / açık sipariş</small></article></div>
            <section class="dashboard-panel"><div class="panel-heading"><div><h2>Bugünkü Tahsilat ve Ödemeler</h2><p>Cari, satış, saha, kasa ve banka işlemlerinden gelen ortak finans hareketleri.</p></div></div><div class="table-scroll"><table><thead><tr><th>Saat</th><th>İşlem</th><th>Cari</th><th>Yöntem</th><th>Açıklama / Belge</th><th>Tutar</th></tr></thead><tbody>${finansBugun.length ? finansBugun.map(x => `<tr><td>${new Date(x.tarih).toLocaleTimeString("tr-TR", { hour: "2-digit", minute: "2-digit" })}</td><td><b>${x.tur === "TAHSILAT" ? "Tahsilat" : "Ödeme"}</b></td><td>${escapeHtml(x.taraf?.unvan || x.taraf?.adSoyad || x.taraf?.kod || "-")}</td><td>${escapeHtml(x.odemeYontemi || "-")}</td><td>${escapeHtml(x.aciklama || x.belgeNo || "-")}</td><td><strong>${para(x.tutar)}</strong></td></tr>`).join("") : '<tr><td colspan="6">Bugün tahsilat veya ödeme hareketi yok.</td></tr>'}</tbody></table></div></section>
    ...
    function sahaIcerikCiz(d) {
        ...
        if (g && !aktif && !k.teslimTarihi && !d.yonetici) {
    ...
    function sahaAksiyonBagli(tur, d) {
        if (tur === "teslim") {
            const kaynak = (d.kasalar || []).find(x => x.sahaKasasi) || (d.kasalar || [])[0];
            ...
            return sahaModal(
                ...
                `<label>Kaynak Saha Kasası<input value="${escapeHtml(kaynak ? `${kaynak.kod} · ${kaynak.ad}` : "Saha kasası")}" disabled></label><label>Ana Kasa<select name="hedefKasaId" required><option value="">Ana kasa seçin</option>${anaKasaOpts}</select></label><label class="full">Teslim Edilen<input name="teslimEdilen" type="number" min="0" step="0.01" value="${Number(d.tesellum?.teslimEdilmesiGereken || 0)}" required></label>`,
