(function () {
    ...
    async function poll() {
        ...
        try {
            ...
            previous = signature;
            panel.innerHTML = '<h2>Sistem uyarıları</h2><p>'+text(summary)+'</p><p>E-posta: '+(d.delivery.emailConfigured?'Yapılandırıldı; sağlayıcı kabulü teslimat garantisi değildir.':'Yapılandırılmadı.')+' WhatsApp: '+(d.delivery.whatsappRecipientSaved?'Alıcı kayıtlı; WhatsApp Business API bağlantısı bekleniyor.':'Yapılandırılmadı.')+'</p><p>Son kontrol: '+text(date(d.checkedAt))+' · Panel açıkken 30 saniyede bir yenilenir.</p>'+
