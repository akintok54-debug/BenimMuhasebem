(function () {
    ...
    async function sistemHatalari() {
        ...
        const rows=data.hatalar.map(x=>'<tr><td>'+tarih(x.createdAt)+'</td><td>'+escapeHtml(x.tenantId?.name || 'Platform')+'<small>'+escapeHtml(x.actorUserId?.email || 'Sistem')+'</small></td><td>'+escapeHtml(x.action)+'<small>'+escapeHtml(x.resource)+' / '+escapeHtml(x.path)+'</small></td><td>'+escapeHtml(x.httpStatus ?? '—')+'<small>'+escapeHtml(x.requestId || '—')+'</small></td><td>'+escapeHtml(x.repeatCount)+' / son 24 saat</td><td>'+(x.resolution?.resolved?'Çözüldü':'Çözülmedi')+'<small>'+escapeHtml(x.resolution?.note || '')+'</small>'+islem(x.resolution?.resolved?'reopen':'resolve',x._id,x.resolution?.resolved?'Tekrar aç':'Çözüldü işaretle')+'</td></tr>');
