...
async function sifreYenile(req, res) {
    try {
        ...
        k.geciciSifre = false;
        await k.save();
        oturumCookieSil(res, k.rol === "BAYI");
...
async function logout(req, res) { const { cookieOku, cookieAdlari, bayiIstegi } = require("../../../services/oturumGuvenligi"); await require("../../platform/services/sessionTelemetry").close(cookieOku(req)[cookieAdlari(req).auth] || req.headers.authorization?.replace(/^Bearer /, "")); oturumCookieSil(res, bayiIstegi(req)); return res.json({ basarili: true, mesaj: "Oturum kapatıldı." }); }
