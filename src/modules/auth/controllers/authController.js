...
async function logout(req, res) { const { cookieOku, AUTH_COOKIE } = require("../../../services/oturumGuvenligi"); await require("../../platform/services/sessionTelemetry").close(cookieOku(req)[AUTH_COOKIE] || req.headers.authorization?.replace(/^Bearer /, "")); oturumCookieSil(res); return res.json({ basarili: true, mesaj: "Oturum kapatıldı." }); }
