...
async function oturumAc(req, res, k) { if (req.platformOturumu && k.rol !== "SUPER_ADMIN") return res.status(403).json({basarili:false,mesaj:"Bu giriş yalnızca platform yöneticileri içindir."}); if (!req.bayiOturumu && k.rol === "BAYI") return res.status(403).json({ basarili:false, mesaj:"Bayi hesabıyla ERP girişi yapılamaz. Bayi portalını kullanın." }); if (req.bayiOturumu && k.rol !== "BAYI") return res.status(403).json({ basarili: false, mesaj: "Bu hesap bayi hesabı değil." }); k.sonGirisTarihi = new Date(); await k.save(); const p = { kullaniciId: k._id.toString(), email: k.email, rol: k.rol, oturumAlani: k.rol === "SUPER_ADMIN" ? "PLATFORM" : k.rol === "BAYI" ? "B2B" : "ERP", tenantId: k.tenantId?.toString() || null }; const token = tokenOlustur(p), csrfToken = oturumCookieYaz(res, token, p.oturumAlani); await require("../../platform/services/sessionTelemetry").observe(req, token, tokenDogrula(token)); req.user = p; return res.json({ basarili: true, mesaj: "Giriş başarılı.", csrfToken, token: process.env.JWT_ALLOW_LEGACY === "false" ? undefined : token, kullanici: { ...guvenliKullanici(k), aktif: k.aktif } }); }

async function login(req, res) {
    try { const body = req.body || {}, kimlik = String(body.kimlik || body.email || "").trim(), sifre = body.sifre; if (!kimlik || !sifre) return res.status(400).json({ basarili: false, mesaj: "E-posta/telefon ve şifre zorunludur." }); const email = kimlik.toLowerCase(), telefonKod = telefonNormalize(kimlik), sorgu = kimlik.includes("@") ? { email } : { telefonNormalize: telefonKod }; const k = await Kullanici.findOne(sorgu).select("+ikiFaktor.gizliAnahtar +ikiFaktor.kurtarmaKodlariHash"); if (!k || !k.aktif || !(await bcrypt.compare(String(sifre), String(k.sifre)))) { if (k) req.user = { kullaniciId: k._id, tenantId: k.tenantId, rol: k.rol }; res.locals.guvenlikOlayi = { kategori: "GIRIS", seviye: "UYARI" }; return res.status(401).json({ basarili: false, mesaj: "E-posta, telefon veya şifre hatalı." }); } if (req.bayiOturumu && k.rol !== "BAYI") return res.status(403).json({ basarili: false, mesaj: "Bu hesap bayi hesabı değil." }); if (req.platformOturumu && k.rol !== "SUPER_ADMIN") return res.status(403).json({basarili:false,mesaj:"Bu giriş yalnızca platform yöneticileri içindir."}); if (k.ikiFaktor?.etkin) return res.json({ basarili: true, ikiFaktorGerekli: true, challengeToken: geciciTokenOlustur({ purpose: "2fa", kullaniciId: k._id.toString() }) }); return oturumAc(req, res, k); } catch (e) { console.error("LOGIN_HATASI", { message: e.message }); return res.status(500).json({ basarili: false, mesaj: "Giriş işlemi sırasında sunucu hatası." }); }
}
...
async function sifreYenile(req, res) {
    try {
        ...
        k.geciciSifre = false;
        await k.save();
        oturumCookieSil(res, k.rol === "SUPER_ADMIN" ? "PLATFORM" : k.rol === "BAYI" ? "B2B" : "ERP");
...
async function logout(req, res) { const { cookieOku, cookieAdlari, bayiIstegi } = require("../../../services/oturumGuvenligi"); await require("../../platform/services/sessionTelemetry").close(cookieOku(req)[cookieAdlari(req).auth] || req.headers.authorization?.replace(/^Bearer /, "")); oturumCookieSil(res, require("../../../services/oturumGuvenligi").oturumAlani(req)); return res.json({ basarili: true, mesaj: "Oturum kapatıldı." }); }
