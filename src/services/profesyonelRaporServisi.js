...
const { raporKapsami, cariBakiyeSatirlari, kalemNet, belgeNet: kayitliBelgeNet, stokTarihSatirlari } = require("./raporMutabakatServisi");
...
function kimlik(value) { return String(value?._id || value || ""); }
...
function stokMaliyetAnalizi(stoklar, hareketler, satisIadeleri, baslangic, bitis, aktifKaynaklar = null, maliyetKapsami = null) {
    ...
    const hareketTarihi = h => new Date(h.tarih || h.createdAt);
    const hareketGunu = h => tarihYaz(hareketTarihi(h));
    const sirali = [...(hareketler || [])].sort((a, b) => hareketGunu(a).localeCompare(hareketGunu(b)) || new Date(a.createdAt || a.tarih) - new Date(b.createdAt || b.tarih) || String(a._id || "").localeCompare(String(b._id || "")));
    const miktarlar = new Map(), stokMap = new Map((stoklar || []).map(x => [`${kimlik(x.urunId)}:${kimlik(x.depoId)}`, Number(x.miktar || 0)]));
    ...
    for (const h of sirali) {
        ...
        const birimMaliyet = maliyet(h);
        const kaynakAktif = !aktifKaynaklar
            || (h.kaynak === "SATIS" ? aktifKaynaklar.satis.has(String(h.kaynakId))
                : h.kaynak === "SIPARIS" ? aktifKaynaklar.siparis.has(String(h.kaynakId)) : true);
        const iptalTersKaydi = /_IPTAL$/.test(String(h.kaynak || ""));
        if (!(birimMaliyet > 0) && kaynakAktif && !iptalTersKaydi) nedenler.add("Maliyeti eksik stok hareketi bulunuyor.");
        const deger = birimMaliyet ? miktar * birimMaliyet : 0, yon = stokYon(h.tip), tarih = hareketTarihi(h), key = `${kimlik(h.urunId)}:${kimlik(h.depoId)}`;
        ...
        const maliyetDahil = !maliyetKapsami || (h.kaynak === "SIPARIS" ? maliyetKapsami.siparis.has(String(h.kaynakId)) : h.kaynak === "SATIS_IADE" ? maliyetKapsami.iade.has(String(h.kaynakId)) : maliyetKapsami.satis.has(String(h.kaynakId)));
        const satisCikisi = maliyetDahil && kaynakAktif && h.tip === "CIKIS" && ["SATIS", "SIPARIS"].includes(h.kaynak), satisIadesi = maliyetDahil && h.tip === "IADE_GIRIS" && h.kaynak === "SATIS_IADE", satisDuzeltmesi = maliyetDahil && kaynakAktif && h.kaynak === "SATIS_DUZELTME" && ["SAYIM_ARTI", "SAYIM_EKSI"].includes(h.tip);
...
async function raporuHesapla(tenantId, query = {}, sabitAralik = null) {
    if (query.raporKodu) { const izinli = new Set(["donem", "baslangic", "bitis", "raporKodu", ...raporKapsami(query.raporKodu, "").filtreAlanlari]); query = Object.fromEntries(Object.entries(query).filter(([k]) => izinli.has(k))); }
    ...
    const [satislar, satisIadeleri, tumSatisIadeleri, alislar, alisIadeleri, stoklar, stokHareketleri, cariHam, donemCariHam, paraSonrasi, digerGelirler, masraflar, portfoy, tenant, cariMusteriler, cariTedarikciler, tumAktifSatislar, tumAktifSiparisler] = await Promise.all([
        ...
        CariHareket.find(cariFiltre).select("tarafTipi tarafId tip tutar bakiyeDegisimi oncekiBakiye sonrakiBakiye durum iptalTarihi tarih odemeYontemi belgeNo aciklama kullaniciId").populate("kullaniciId", "adSoyad").lean(),
        CariHareket.find({ ...cariFiltre, tarih, durum: { $ne: "IPTAL" } }).select("tarafTipi tarafId tip tutar bakiyeDegisimi tarih odemeYontemi belgeNo aciklama kullaniciId").populate("kullaniciId", "adSoyad").sort({ tarih: -1 }).lean(),
        ParaHareket.find({ tenantId, tarih: { $gt: aralik.bitis }, ...(sube ? { hesapId: { $in: hesapIds } } : {}) }).select("hesapTipi hesapId tip tutar").lean(),
        ParaHareket.find({ tenantId, tarih, tip: "GIRIS", kaynak: "MANUEL", durum: { $ne: "IPTAL" }, ...(sube ? { hesapId: { $in: hesapIds } } : {}) }).select("tutar tarih aciklama belgeNo hesapTipi hesapId").lean(),
        Masraf.find(masrafFiltre).select("kategori tutar tarih aciklama fisNo hesapTipi hesapId paraBirimi").sort({ tarih: -1 }).lean(),
        CekSenetPortfoy.find({ tenantId, durum: "PORTFOYDE", ...(musteriId ? { musteriId } : {}) }).select("tur tutar belgeNo vadeTarihi musteriId durum").populate("musteriId", "kod unvan adSoyad").lean(),
        Tenant.findById(tenantId).select("name firmaBilgileri.unvan").lean(),
        Musteri.find({ tenantId }).select("kod unvan adSoyad bakiye createdAt").lean(), Tedarikci.find({ tenantId }).select("kod unvan adSoyad bakiye createdAt").lean(),
        Satis.find({ tenantId, durum: { $ne: "IPTAL" } }).select("_id").lean(), Siparis.find({ tenantId, durum: { $ne: "IPTAL" } }).select("_id satisId").lean()
    ]);
    ...
    for (const s of satislar) {
        const kalemler = uygunKalemler(s), belgeNet = yuvarla(kayitliBelgeNet(s, kalemler, urunKisitli)); toplamSatisGeliri += belgeNet; gun(s.tarih).satis += belgeNet;
        ...
        mapTopla(temsilciRaporu, String(s.kullaniciId?._id || s.kullaniciId || "atanmamis"), { ad: s.kullaniciId?.adSoyad || "Atanmamış", belgeSayisi: 0, satis: 0, iade: 0, tahsilat: 0 }, { belgeSayisi: 1, satis: belgeNet, tahsilat: 0 });
    ...
    for (const i of satisIadeleri) { const kalemler = uygunKalemler(i), net = yuvarla(kayitliBelgeNet(i, kalemler, urunKisitli)); satisIadeToplami += net; gun(i.tarih).satis -= net; mapTopla(musteriRaporu, String(i.musteriId?._id || i.musteriId), { kod: i.musteriId?.kod || "", ad: ad(i.musteriId), belgeSayisi: 0, satis: 0, iade: 0 }, { iade: net }); mapTopla(temsilciRaporu, String(i.kullaniciId?._id || i.kullaniciId || "atanmamis"), { ad: i.kullaniciId?.adSoyad || "Atanmamış", belgeSayisi: 0, satis: 0, iade: 0, tahsilat: 0 }, { iade: net }); for (const k of kalemler) { const u = urunMap.get(String(k.urunId)); mapTopla(urunRaporu, String(k.urunId), { kod: u?.kod || "", urun: u?.ad || "-", marka: u?.marka || "", kategori: u?.kategori || "", satilanMiktar: 0, iadeMiktari: 0, netSatis: 0, maliyet: 0, kar: 0 }, { iadeMiktari: k.miktar, netSatis: -kalemNet(k) }); } }
    for (const a of alislar) { const kalemler = uygunKalemler(a), net = yuvarla(kayitliBelgeNet(a, kalemler, urunKisitli)); toplamAlis += net; gun(a.tarih).alis += net; mapTopla(tedarikciRaporu, String(a.tedarikciId?._id || a.tedarikciId), { kod: a.tedarikciId?.kod || "", ad: ad(a.tedarikciId), belgeSayisi: 0, alis: 0, iade: 0 }, { belgeSayisi: 1, alis: net }); for (const k of kalemler) { const u = urunMap.get(String(k.urunId)); mapTopla(alisUrunRaporu, String(k.urunId), { kod: u?.kod || "", urun: u?.ad || "-", alinanMiktar: 0, iadeMiktari: 0, alis: 0, iade: 0 }, { alinanMiktar: k.miktar, alis: kalemNet(k) }); } }
    for (const i of alisIadeleri) { const kalemler = uygunKalemler(i), net = yuvarla(kayitliBelgeNet(i, kalemler, urunKisitli)); alisIadeToplami += net; gun(i.tarih).alis -= net; mapTopla(tedarikciRaporu, String(i.tedarikciId?._id || i.tedarikciId), { kod: i.tedarikciId?.kod || "", ad: ad(i.tedarikciId), belgeSayisi: 0, alis: 0, iade: 0 }, { iade: net }); for (const k of kalemler) { const u = urunMap.get(String(k.urunId)); mapTopla(alisUrunRaporu, String(k.urunId), { kod: u?.kod || "", urun: u?.ad || "-", alinanMiktar: 0, iadeMiktari: 0, alis: 0, iade: 0 }, { iadeMiktari: k.miktar, iade: kalemNet(k) }); } }
    const aktifKaynaklar = { satis: new Set(tumAktifSatislar.map(x => String(x._id))), siparis: new Set(tumAktifSiparisler.map(x => String(x._id))) };
    const maliyetKapsami = { satis: new Set(satislar.map(x => String(x._id))), iade: new Set(satisIadeleri.map(x => String(x._id))), siparis: new Set(tumAktifSiparisler.filter(x => satislar.some(s => String(s._id) === String(x.satisId))).map(x => String(x._id))) };
    const maliyetAnalizi = stokMaliyetAnalizi(stoklar, stokHareketleri, tumSatisIadeleri, aralik.baslangic, aralik.bitis, aktifKaynaklar, maliyetKapsami);
    ...
    const tahsilatlar = donemCari.filter(x => x.tarafTipi === "MUSTERI" && x.tip === "TAHSILAT" && (!temsilciId || kimlik(x.kullaniciId) === String(temsilciId)) && (!cariOdemeTipi || x.odemeYontemi === cariOdemeTipi));
    const odemeler = donemCari.filter(x => x.tarafTipi === "TEDARIKCI" && x.tip === "ODEME" && (!cariOdemeTipi || x.odemeYontemi === cariOdemeTipi));
    for (const h of tahsilatlar) { gun(h.tarih).tahsilat += Number(h.tutar || 0); const key = kimlik(h.kullaniciId) || "atanmamis"; mapTopla(temsilciRaporu, key, { ad: ad(h.kullaniciId), belgeSayisi: 0, satis: 0, iade: 0, tahsilat: 0 }, { tahsilat: h.tutar }); } for (const h of odemeler) gun(h.tarih).odeme += Number(h.tutar || 0); for (const m of masraflar) gun(m.tarih).gider += Number(m.tutar || 0);
    const temsilciSatirlari = [...temsilciRaporu.values()].map(x => ({ ...x, netSatis: yuvarla(x.satis - x.iade) })).sort((a, b) => b.netSatis - a.netSatis);
    const cariBakiyeler = [...cariBakiyeSatirlari(cariMusteriler.filter(x => !musteriId || String(x._id) === String(musteriId)), cariHam, "MUSTERI", aralik.bitis), ...cariBakiyeSatirlari(cariTedarikciler.filter(x => !tedarikciId || String(x._id) === String(tedarikciId)), cariHam, "TEDARIKCI", aralik.bitis)];
    const musteriAlacakSatirlari = cariBakiyeler.filter(x => x.tarafTipi === "MUSTERI" && x.bakiye > 0).sort((a, b) => b.bakiye - a.bakiye), tedarikciBorcSatirlari = cariBakiyeler.filter(x => x.tarafTipi === "TEDARIKCI" && x.bakiye > 0).sort((a, b) => b.bakiye - a.bakiye);
    ...
    const baslangicStokSatirlari = stokTarihSatirlari(stoklar, stokHareketleri, aralik.baslangic, true, maliyetAnalizi.stokDegeriGuvenilir);
    const bitisStokSatirlari = stokTarihSatirlari(stoklar, stokHareketleri, aralik.bitis, false, maliyetAnalizi.stokDegeriGuvenilir);
    ...
    for (const kod of ["faaliyetKari", "netKarZarar"]) raporlar[kod].satirlar = [{ kalem: "Net Satis", tutar: degerler.netSatislar }, { kalem: "SMM", tutar: degerler.satilanMalinMaliyeti === null ? null : -degerler.satilanMalinMaliyeti }, { kalem: "Diger Gelir", tutar: degerler.digerGelirler }, { kalem: "Gider", tutar: -degerler.toplamGiderler }];
    for (const [kod, r] of Object.entries(raporlar)) Object.assign(r, raporKapsami(kod, aralik.bitisYazi));
    const ozet = { toplamSatis: degerler.toplamSatisGeliri, netSatis: degerler.netSatislar, toplamAlis: degerler.toplamAlis, tahsilat: toplamEsleme.tahsilatRaporu, odeme: toplamEsleme.odemeRaporu, gider: degerler.toplamGiderler, brutKar: degerler.brutKar, netKarZarar: degerler.netKarZarar, musteriAlacagi: toplamEsleme.musteriAlacaklari, tedarikciBorcu: toplamEsleme.tedarikciBorclari, kasa: toplamEsleme.kasaBakiyesi, banka: toplamEsleme.bankaBakiyesi, stokDegeri: toplamEsleme.stokDegeri };
