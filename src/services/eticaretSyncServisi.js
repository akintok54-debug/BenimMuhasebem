...
const disFiyat = require('./disKanalFiyatServisi');
...
function ideasoftSiparisSatirlariniNormalizeEt(order) {
    ...
    return rows.map(line => {
        ...
        return {
            raw: line,
            ...(line.taxIncluded === undefined ? {} : {kdvDahil:disFiyat.dahil(line.taxIncluded)}),
...
async function stokFiyatGonder(job, connection, adapter) {
    const mappings = await MarketplaceProductMapping.find({ tenantId: job.tenantId, storeConnectionId: connection._id }).populate({path:"productId",match:{tenantId:job.tenantId}}).lean();
    ...
    for (const mapping of mappings) {
        ...
        const quantity = urun.aktif === false ? 0 : Math.max(0, Math.floor((stokMap.get(String(urun._id)) || 0) - Number(mapping.reservedStock || 0) - Number(mapping.safetyStock || 0)));
        const prices=disFiyat.kanalFiyati(urun,connection.provider),{salePrice,listPrice,vatRate}=prices,signature=require('node:crypto').createHash('sha256').update(JSON.stringify(prices)).digest('hex'),stockChanged=mapping.lastStockSent===null||Number(mapping.lastStockSent)!==quantity,priceChanged=mapping.lastPriceSignature!==signature;
        if ((job.type === "STOCK_PUSH" && !stockChanged) || (job.type === "PRICE_PUSH" && !priceChanged)) continue;
        items.push({ externalProductId: mapping.externalProductId, barcode: mapping.externalBarcode, quantity, salePrice, listPrice, vatRate });
        degisen.push({ mapping, quantity, salePrice, signature });
    }
    if (!items.length) return { processed: mappings.length, success: 0, skipped: mappings.length };
    const response=await tekrarDene(() => job.type === "PRICE_PUSH" ? adapter.updatePrices(items) : adapter.updateStock(items));
    if(response?.batchRequestId){await MarketplaceProductMapping.updateMany({_id:{$in:degisen.map(x=>x.mapping._id)},tenantId:job.tenantId},{$set:{syncStatus:'PENDING',lastError:'Sağlayıcı toplu işlem sonucunun doğrulanması bekleniyor: '+response.batchRequestId}});return {processed:mappings.length,success:0,pending:items.length,batchRequestId:response.batchRequestId};}
    await Promise.all(degisen.map(({ mapping, quantity, salePrice, signature }) => MarketplaceProductMapping.updateOne({ _id: mapping._id, tenantId: job.tenantId }, { $set: { ...(job.type === "PRICE_PUSH" ? {lastPriceSent:salePrice,lastPriceSignature:signature} : {lastStockSent:quantity}), lastSyncedAt: new Date(), syncStatus: "SYNCED", lastError: "" } })));
...
async function urunGonder(job, connection, adapter) {
    const mappings = await MarketplaceProductMapping.find({ tenantId: job.tenantId, storeConnectionId: connection._id }).populate({path:"productId",match:{tenantId:job.tenantId}}).lean(), productIds = mappings.map(x => x.productId?._id).filter(Boolean);
    ...
    const items = mappings.map(mapping => {
        ...
        const {salePrice,listPrice,vatRate}=disFiyat.kanalFiyati(product,connection.provider), quantity = Math.max(0, Math.floor((stockMap.get(String(product._id)) || 0) - Number(mapping.reservedStock || 0) - Number(mapping.safetyStock || 0)));
...
async function siparisleriAl(job, connection, adapter) {
    ...
    for (const pkg of packages) {
        ...
        let incomingCalculation,incomingError;
        if(connection.provider==='IDEASOFT')try{incomingCalculation=disFiyat.siparisHesapla(normalizedLines,ilkDeger(pkg.generalAmount,pkg.totalPrice,pkg.totalAmount,pkg.total));}catch(e){incomingError=e;}
        for (const [lineIndex,line] of normalizedLines.entries()) { const barcode = line.externalBarcode, sku = line.externalSku, externalProductId = line.externalProductId, externalFilters = [...(externalProductId ? [{ externalProductId }] : []), ...(barcode ? [{ externalBarcode: barcode }] : []), ...(sku ? [{ externalSku: sku }] : [])], productFilters = [...(barcode ? [{ barkod: barcode }] : []), ...(sku ? [{ kod: sku.toUpperCase() }] : [])]; if (!externalFilters.length && !productFilters.length) { missing.push(String(line.raw?.id || "BARKOD_SKU_YOK")); lines.push({ ...line, urunId: null }); continue; } const mapping = externalFilters.length ? await MarketplaceProductMapping.findOne({ tenantId: job.tenantId, storeConnectionId: connection._id, $or: externalFilters }).lean() : null; let product = mapping?.productId ? await Urun.findOne({ _id: mapping.productId, tenantId: job.tenantId }).lean() : productFilters.length ? await Urun.findOne({ tenantId: job.tenantId, $or: productFilters }).lean() : null; if (!product && connection.provider === "IDEASOFT" && incomingCalculation && (barcode || sku)) { const kod = (sku || barcode || `IDEA-${externalProductId}`).toUpperCase().slice(0, 60), fiyat = incomingCalculation.kalemler[lineIndex].birimFiyat; try { product = await Urun.findOneAndUpdate({ tenantId: job.tenantId, kod }, { $setOnInsert: { tenantId: job.tenantId, kod, barkod: barcode, ad: `IdeaSoft Ürün ${sku || barcode}`, satisFiyati: fiyat, perakendeFiyati: fiyat, kdv: Math.max(0, Number(line.vergi ?? 20)), notlar: "IdeaSoft siparişinden otomatik oluşturuldu" } }, { upsert: true, new: true, setDefaultsOnInsert: true, runValidators: true }).lean(); } catch (error) { product = null; } } if (!product) { missing.push(barcode || sku || externalProductId || String(line.raw?.id || "")); lines.push({ ...line, urunId: null }); continue; } lines.push({ urunId: product._id, miktar: line.miktar, birimFiyat: line.birimFiyat, externalSku: sku, externalBarcode: barcode, iskonto: line.iskonto, vergi: line.vergi, kdvDahil:line.kdvDahil }); }
        ...
        try {
            ...
            const calculated=incomingCalculation,erpLines=calculated.kalemler.map((x,i)=>({...x,urunId:lines[i].urunId}));
            const erpOrder = await Siparis.findOneAndUpdate({ tenantId: job.tenantId, siparisNo: erpNo }, { $setOnInsert: { tenantId: job.tenantId, siparisNo: erpNo, tarih: orderDate, musteriId: customer._id, depoId: depot._id, kalemler: erpLines, araToplam: calculated.araToplam, toplamKdv: calculated.toplamKdv, genelToplam: calculated.genelToplam, paraBirimi: ["TRY","USD","EUR"].includes(currency) ? currency : "TRY", sevkAdresi: [pkg.shippingAddress?.address, pkg.shippingAddress?.district, pkg.shippingAddress?.locationName].filter(Boolean).join(" · "), durum: pkg.status === "cancelled" ? "IPTAL" : "ONAYLANDI", notlar: `IdeaSoft siparişi ${externalOrderId}` } }, { upsert: true, new: true, runValidators: true });
    ...
    for (let page = 1; page <= maxPages; page++) {
        ...
        for (const external of products) {
            ...
            if (!product) {
                ...
                const {net:satisFiyati,kdv}=disFiyat.ideasoftNet(external);
                try {
...
async function ideasoftPilotTest({ tenantId, connection, userId }) {
    ...
    try {
        ...
        const mapping = await MarketplaceProductMapping.findOne({ tenantId, storeConnectionId: secured._id, productId: { $ne: null }, externalProductId: { $ne: "" } }).populate({path:"productId",match:{tenantId:job.tenantId}}).lean();
        if (mapping?.productId) { const stockRows = await Stok.aggregate([{ $match: { tenantId, urunId: mapping.productId._id } }, { $group: { _id: null, quantity: { $sum: "$miktar" } } }]), quantity = Math.max(0, Math.floor(Number(stockRows[0]?.quantity || 0))), salePrice = Number(mapping.productId.satisFiyati || 0),vatRate=Number(mapping.productId.kdv??20); await adapter.updateStock([{ externalProductId: mapping.externalProductId, quantity }]); stages.push({ step: "STOCK_UPDATE_ONE", success: true }); await adapter.updatePrice([{ externalProductId: mapping.externalProductId, salePrice, vatRate }]); stages.push({ step: "PRICE_UPDATE_ONE", success: true }); }
...
async function isiCalistir(jobId) {
    ...
    try {
        ...
        job.processedCount = sonuc.processed || 0; job.successCount = sonuc.success || 0; job.errorCount = sonuc.errors || 0; job.errors = guvenliDetay(sonuc.errorDetails || []); job.status = sonuc.pending ? "PARTIAL" : sonuc.errors ? (sonuc.success ? "PARTIAL" : "FAILED") : "SUCCESS";
