...
class IdeaSoftAdapter extends MarketplaceAdapter {
    ...
    async updateProduct(id, changes) { if (!id) throw new IntegrationError("VALIDATION_ERROR", "IdeaSoft ürün kimliği zorunludur.", { status: 400 }); const current = await this.request(`/admin-api/products/${encodeURIComponent(id)}`); return this.request(`/admin-api/products/${encodeURIComponent(id)}`, { method: "PUT", body: { ...current, ...(typeof changes === 'function' ? changes(current) : changes), id: Number(id) || current.id } }); }
    ...
    updatePrice(items) { if (!Array.isArray(items) || !items.length) return Promise.resolve({ skipped: true, reason: "UNCHANGED" }); return Promise.all(items.map(item => this.updateProduct(item.externalProductId, current => require('../../services/disKanalFiyatServisi').ideasoftDegisiklik(current,item))) ); }
