...
class TrendyolAdapter extends MarketplaceAdapter {
    ...
    updateStock(items) { const { sellerId } = this.auth(); if (!Array.isArray(items) || !items.length) return { skipped: true, reason: "UNCHANGED" }; return this.request(`/integration/inventory/sellers/${encodeURIComponent(sellerId)}/products/price-and-inventory`, { method: "POST", body: { items: items.map(({barcode,quantity})=>({barcode,quantity})) } }); }
    updatePrices(items) { const { sellerId } = this.auth(); if (!Array.isArray(items) || !items.length) return { skipped: true, reason: "UNCHANGED" }; return this.request(`/integration/inventory/sellers/${encodeURIComponent(sellerId)}/products/price-and-inventory`, { method: "POST", body: { items: items.map(({barcode,salePrice,listPrice})=>({barcode,salePrice,listPrice})) } }); }
